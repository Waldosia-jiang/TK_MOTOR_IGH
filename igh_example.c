...
int target_torque = 0;                                   // CST模式目标转矩，通过-t参数设置（外部变量，供motion.c使用）
        ...
        if (control_cycle_counter >= ethercat_cycles_per_control) {
            ...
            for(index = 0; index < slave_count; ++index) {
                if(is_servo_slave[index]){
                    if(is_move) {
                        // 根据-v/-t参数设置正确的运行模式（PDO模板固定为0x1607/0x1A07，RW模板）
                        unsigned char operation_mode = 8;  // 默认CSP模式
                        // 如果设置了-t参数（target_torque != 0），则使用CST模式
                        if (target_torque != 0) {
                            operation_mode = 4;  // CST模式
                        } else if (target_velocity_cnt != 0) {
                            operation_mode = 3;  // CSV模式
                        } else {
                            operation_mode = 8;  // CSP模式（默认）
                        }
                        motion_set_operation_mode(index, operation_mode);  // 设置运行模式
                        
                        // 只有在通信正常时才更新状态机，避免通信异常时误下闸
                        if (comm_ok) {
                            motion_power_state_machine(index, 1);    // 上电状态机
                            motion_fault_recovery(index);             // 故障检测和恢复
                        }
                        // 运动控制算法：轨迹规划、位置计算等（即使通信异常也继续，保持目标位置）
