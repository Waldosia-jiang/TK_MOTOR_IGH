        ...
        switch(operation_mode) {
            ...
            case 3:  // CSV模式 (Cyclic Synchronous Velocity)
                ...
                if(offset[i].target_velocity >= 0 && offset[i].target_velocity < 0x0000ffff) {
                    ...
                    // 调试：定期打印写入的速度值（每1000个周期）
                    static int csv_debug_counter = 0;
                    if (++csv_debug_counter % 1000 == 0) {
            ...
            case 4:  // CST模式 (Cyclic Synchronous Torque)
                ...
                if(offset[i].target_torque >= 0 && offset[i].target_torque < 0x0000ffff) {
                    ...
                    // 调试：定期打印写入的转矩值（每1000个周期）
                    static int cst_debug_counter = 0;
                    if (++cst_debug_counter % 1000 == 0) {
...
void motion_power_state_machine(int slv_num, int is_on)
{
    ...
    if(is_on) //上电
    {
        ...
        {
            // 0x00: 未就绪状态
            // 如果之前是正常运行状态（0x07），现在变成0x00，说明可能是通信异常导致的下电
            // 直接发送0x06重新上电，但不修改is_power_on标志，避免误判
            if (motion_data[slv_num].is_power_on && last_status_low_val == 0x07) {
                // 从正常运行状态变成0x00，说明驱动器可能因为通信异常而下电了
                // 直接发送0x06重新上电，不修改is_power_on
                *cw = 0x06;
                if (should_print) {
                    printf("轴 %d: ⚠️ 状态字从0x07变为0x00（通信异常导致下电？），发送0x06重新上电 (状态=0x%04X)\n", slv_num, *sw);
                }
            } else if (motion_data[slv_num].is_power_on) {
                // 已经上电运行，但之前不是0x07，可能是短暂的状态变化，保持0x0F
                *cw = 0x0f;
                if (should_print && last_status_low_val == 0x07) {
                    printf("轴 %d: 🔍 状态字从0x07变为0x00（通信抖动？），保持控制字0x0F (状态=0x%04X)\n", slv_num, *sw);
                }
            } else {
                // 初始上电状态，发送0x06继续上电流程
                *cw = 0x06;
                if (should_print) {
                    printf("轴 %d: 🔍 初始上电 - 状态字0x00，发送控制字0x06 (状态=0x%04X)\n", slv_num, *sw);
                }
            }
...
void motion_process_slave_motion_cst(int slv_num)
{
    ...
    // 使用 -t 作为位置增量（cnt），根据当前位置和速度误差计算目标转矩
    // 首次进入CST模式时，记录初始位置，并固定期望位置 = 初始位置 + 位置增量
    ...
    // 固定期望位置 = 初始位置 + 位置增量（cnt）
    int32_t pos_desired_cnt = g_cst_initial_position[slv_num] + target_torque;
    ...
    if (torque_cmd > 32767.0) torque_cmd = 32767.0;
    if (torque_cmd < -32768.0) torque_cmd = -32768.0;
    motion_data[slv_num].target_torque = (short)torque_cmd;
    
    // 首次设置转矩时打印信息
    static int cst_torque_initialized[MAX_SLAVE_NUMBER] = {0};
    if (!cst_torque_initialized[slv_num]) {
        ...
        cst_torque_initialized[slv_num] = 1;
    }
}
