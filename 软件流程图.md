...
| `-t` | CST模式位置增量 | - | 力位混合控制位置增量（单位：脉冲cnt，内部通过转矩输出实现位置闭环） |
...
for (每个从站 i) {
    ...
    3. 如果是伺服:
       ...
       - 根据-v/-t参数决定CST/CSV/CSP模式（-t 为位置增量，内部通过转矩输出实现力位混合控制）
       - 注册PDO条目到域
       - 配置SDO参数 (速度、加速度、减速度)
    4. 配置分布式时钟 (DC)
}
...
- **CST模式**：力位混合控制，使用-t参数设置位置增量（cnt），内部根据位置误差计算转矩输出
...
| `target_torque` | int | CST模式位置增量（cnt），通过 `-t` 参数设置，内部根据该增量进行力位混合控制 |
...
### 9.5 CST模式 - 力位混合控制（位置 + 力矩）

```
使用-t参数设置位置增量（单位：脉冲cnt）
  target_torque 作为位置增量（cnt）参与计算
  控制算法根据“初始位置 + 位置增量”和当前位置、速度误差计算目标转矩
  持续输出转矩，实现位置闭环 + 力矩输出的混合控制
```
